...
The guard is published as an open-source Kotlin/JVM library, [ssrf-guard](https://github.com/Andreydemo/ssrf-guard) (Apache-2.0): the URL policy, the address classifier including the IPv6 transition forms and the RFC 8215 local-use NAT64 prefix, the public-only resolver, and an OkHttp `Dns` adapter, with the test vectors below as its test suite. A [technical report](https://github.com/Andreydemo/ssrf-guard/blob/main/docs/report.pdf) with the threat model, design and evaluation ships with each release.
